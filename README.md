...
親指シフト（Nicola）を、無変換・変換キーのないUSキーボードでも使えるように、**中指同時シフト化**したものです。

（なお、中指同時シフト配列の代表例としては、新下駄配列などがあります。それらと似た感覚で打てるようになっています。また、後述する SandS = Space and Shift は薙刀式などでも採用されており、キーボード種類を選ばない親指シフトとして用いられています。SandSを使うかどうかはオプションです。）
